...
const MidPoint = () => {
  ...
  console.log("test for location lat &lon from a midpoiny",locate.state[0].lat)
  ...
  // }, [locations]);
      ...
      //   const midpointLongitude = totalLongitude / locations.length;
        ...
        let latitudes= locations.map(location => parseFloat(location.lat) * Math.PI / 180);
        let longitudes=locations.map(location => parseFloat(location.long) * Math.PI / 180);
